...
| `net.h`, `net.cpp` | Wire-protocol message types, `version` / `verack` handshake, inv / getdata / block / tx semantics. We don't ship Satoshi's framing (libp2p replaces it) but the *application-level* message taxonomy in `p2p/messages.go` matches one-for-one. |
